...
| A5 | `[lane:gate] [size:M]` `scripts/retier_holdings.py`: dry-run default, `--apply` refuses a dirty tree, refuses on manifest/state disagreement before any write, tier from `family_tier_overrides` else default | (a) `0 records to retier` + `git status` unchanged, (b) fixture flips exactly 1 record then PASS, (c) dirty-tree `--apply` exits nonzero saying `dirty`, (d) broken-base fixture: tree hash identical after refusal, (e) suite `N/N passed` | A4 | cc:wip |

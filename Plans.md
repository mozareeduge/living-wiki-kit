...
| W2 | `[lane:gate] [tdd:required] [size:S]` MCP tool `wiki_context_pack` wrapping `context_pack.py` (≤ 30 records, ≤ 16k tokens, reason per record) | (a) tool listed in `tools/list`, (b) dispatch test returns records with reasons, (c) suite `N/N passed` | S3 | cc:wip |

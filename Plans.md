...
| A3 | `[lane:gate] [size:S]` `CORPUS_STATE.json` gains `held_artifact_count: 0` + `holdings_by_tier`; SYSTEM_DESIGN pins the vocabulary (`source_material_count` = manifest rows, never redefined) | (a) `0 0` printed for held + tier sum, (b) `validate_repo.py --full` PASS, (c) suite `N/N passed`, (d) the distinguishing sentence present | A2 | cc:wip |

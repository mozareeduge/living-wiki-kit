...
- [ ] 1.1 Add a deterministic `__main__` runner to
...
- [ ] 1.2 RED: add table-driven tests for `check_entry_pages(root, state,
...
- [ ] 1.3 GREEN: implement the extracted helper in
...
- [ ] 2.1 Add the exact `Current corpus snapshot:` and
...
- [ ] 2.2 Update SYSTEM_DESIGN.md §6 to document the four-page freshness and
      ...
      facts.”
- [ ] 2.3 Add `filename:` and its one-line description to
...
- [ ] 2.4 Replace wiki-reconcile's hardcoded baseline with a run-start read of
...
- [ ] 2.5 Update wiki-intake step 10 to refresh the four markers in the same
...
- [ ] 3.1 RED: extend the same plain-Python suite with a temporary-copy smoke
...
- [ ] 3.2 GREEN: update `scripts/instantiate.py` to validate fresh-state
...
- [ ] 4.1 RED: add an integration test proving normal `validate()` calls
...
- [ ] 5.1 In a disposable local clone, set `core.hooksPath=.githooks`, change
...
- [ ] 5.2 Review `git diff --check`, the bounded file list, and the full diff.
      ...
      staged.
- [ ] 5.3 If releasing as 1.1.0, update SYSTEM_DESIGN.md `system_version`,
